...
windows: ## Build a 64-bit Windows executable
	@GOOS=windows GOARCH=amd64 go build -o ./build/game.exe .

mac: ## Build a 64-bit ARM (Apple Silicon) binary
	@GOOS=darwin GOARCH=arm64 go build -o ./build/game .

linux: ## Build a 64-bit Linux binary
	@GOOS=linux GOARCH=amd64 go build -o ./build/game .

mod: ## Install modules
	@go mod tidy

play: ## Play the game (for development only!)
	@go run main.go
